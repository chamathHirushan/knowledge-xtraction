import pandas as pd
from graph_construction.edc.extract import Extractor
from graph_construction.edc.schema_definition import SchemaDefiner
from graph_construction.edc.schema_canonicalization import SchemaCanonicalizer
from graph_construction.edc.entity_extraction import EntityExtractor
import graph_construction.edc.utils.llm_utils as llm_utils
from typing import List
from graph_construction.edc.utils.e5_mistral_utils import MistralForSequenceEmbedding
from transformers import AutoModelForCausalLM, AutoTokenizer
from graph_construction.edc.schema_retriever import SchemaRetriever
...
class EDC:
    ...
    def oie(
        ...
        if llm_utils.is_model_openai(self.oie_llm_name):
            extractor = Extractor(openai_model=self.oie_llm_name)
        elif llm_utils.is_model_gemini(self.oie_llm_name):
            extractor = Extractor(gemini_model=self.oie_llm_name)
        else:
    ...
    def schema_definition(self, input_text_list: List[str], oie_triplets_list: List[List[str]], free_model=False):
        ...
        if llm_utils.is_model_openai(self.sd_llm_name):
            schema_definer = SchemaDefiner(openai_model=self.sd_llm_name)
        elif llm_utils.is_model_gemini(self.sd_llm_name):
            schema_definer = SchemaDefiner(gemini_model=self.sd_llm_name)
        else:
    ...
    def schema_canonicalization(
        ...
        elif llm_utils.is_model_gemini(self.sc_llm_name):
            schema_canonicalizer = SchemaCanonicalizer(self.schema, sc_embedder, verify_gemini_model=self.sc_llm_name)
        else:
    ...
    def construct_refinement_hint(
        ...
        if llm_utils.is_model_openai(self.ee_llm_name):
            entity_extractor = EntityExtractor(openai_model=self.sd_llm_name)
        elif llm_utils.is_model_gemini(self.ee_llm_name):
            entity_extractor = EntityExtractor(gemini_model=self.ee_llm_name)
        else:
            # Load the HF model for Schema Definition
    ...
    def extract_kg(self, input_text_list: List[str], output_dir: str = None, refinement_iterations=0):
        ...
        for iteration in range(refinement_iterations + 1):
            ...
            print ("oie_triplets_list:", oie_triplets_list)
