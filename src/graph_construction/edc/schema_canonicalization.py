...
class SchemaCanonicalizer:
    ...
    def __init__(
        ...
        verify_gemini_model: AutoTokenizer = None,
    ) -> None:
        ...
        assert verify_openai_model is not None or verify_gemini_model is not None or (verify_model is not None and verify_tokenizer is not None)
        ...
        self.verifier_gemini_model = verify_gemini_model
    ...
    def llm_verify(
        ...
        if self.verifier_openai_model is not None:
            verification_result = llm_utils.openai_chat_completion(
                self.verifier_openai_model, None, messages, max_tokens=1
            )
        elif self.verifier_gemini_model is not None:
            verification_result = llm_utils.gemini_chat_completion(self.verifier_gemini_model, None, messages, max_tokens=1)
        else:
